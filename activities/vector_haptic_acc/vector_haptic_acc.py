...
class VectorHapticAcc(Activity):
    ...
    def __init__(self, body_point_array, ui, **kwargs) -> None:
        ...
        stage_0 = {}
        stage_0["target_1"] = cf.new_button(50, (255, 0, 0, 120), random.uniform(-0.7, 0.7)*PIXEL_SCALE+PIXEL_X_OFFSET, random.uniform(0.0, -0.8)*PIXEL_SCALE+PIXEL_Y_OFFSET, precision=50, func=self.target_1_func, target_pts=[self.POINT])
        stage_0["bubble_1"] = cf.new_hand_bubble(0, 0, self.POINT, 30, (255, 0, 0, 120))
    ...
    def handle_frame(self, **kwargs) -> None:
        ...
        self.index += 1
        self.current_pos = np.array([0, self.persist[SKELETON].skeleton_array[self.POINT][0], self.persist[SKELETON].skeleton_array[self.POINT][1]]) #Current Pos
