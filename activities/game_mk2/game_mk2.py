...
class GameMkII(Activity):
    ...
    def __init__(self, body_point_array, **kwargs) -> None:
        ...
        stage_2 = {}
        stage_2["name_target_1"] = ButtonComponent(50, mkBrush(0, 255, 0, 120), 
            horz_starting_pt, height, func=self.temp_func, target_pts=[15], precision=0.1)
        stage_2["name_target_2"] = ButtonComponent(50, mkBrush(0, 255, 0, 120), 
            horz_starting_pt+spacing, height, func=self.temp_func, target_pts=[15], precision=0.1)
        stage_2["name_target_3"] = ButtonComponent(50, mkBrush(0, 255, 0, 120), 
            horz_starting_pt+spacing*2, height, func=self.temp_func, target_pts=[15], precision=0.1)
        stage_2["name_target_4"] = ButtonComponent(50, mkBrush(0, 255, 0, 120), 
            horz_starting_pt+spacing*3, height, func=self.temp_func, target_pts=[15], precision=0.1)
        ...
        stage_2["name_label_4"] = TextComponent(
    ...
    def submit_score_func(self):
        ...
        file.write("tset")#f"{self.LETTER_SELECT[self.name_input_1_index % len(self.LETTER_SELECT)]}{self.LETTER_SELECT[self.name_input_2_index % len(self.LETTER_SELECT)]}{self.LETTER_SELECT[self.name_input_3_index % len(self.LETTER_SELECT)]},{self.score}\n")
        file.close()
